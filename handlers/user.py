...
async def add_cart(callback_query: types.CallbackQuery, session: AsyncSession):
    ...
    if promo.quantity == 0:
        ...
        image, kbds = await payment(session, data[-2], callback_query.from_user.id)
...
async def process_show_promocodes(callback_query: types.CallbackQuery, session: AsyncSession):
