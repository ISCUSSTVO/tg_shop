...
async def orm_update_catalog(session: AsyncSession, promocode: str, field_name: str, new_value: str):
    query = update(Catalog).where(Catalog.name == promocode).values({field_name: new_value})
    result = await session.execute(query)
    return result
